...
namespace krabs {
    ...
    public:
         ...
         * </remarks>
         */
		inline property::property(const EVENT_PROPERTY_INFO &prop, const schema &schema);
		...
		PROPERTY_FLAGS flags() const;
		std::vector<property> get_subproperties() const;

    private:
		const EVENT_PROPERTY_INFO &prop_;
    ...
    , schema_(schema)
    {
		const wchar_t *pName = reinterpret_cast<const wchar_t*>(
			...
			prop_.NameOffset);
		type_ = (_TDH_IN_TYPE)prop_.nonStructType.InType;
		name_ = std::wstring(pName);
	}

	inline std::vector<property> property::get_subproperties() const
	{
		std::vector<property> props;
		if (prop_.Flags & PropertyStruct) {
			for (int i = 0; i < prop_.structType.NumOfStructMembers; ++i) {
				const auto &curr_prop = schema_.pSchema_->EventPropertyInfoArray[prop_.structType.StructStartIndex + i];
				props.emplace_back(property(curr_prop, schema_));
			}
		}
	...
	inline PROPERTY_FLAGS property::flags() const
	{
		return prop_.Flags;
	}
    ...
    inline property property_iterator::get_property(size_t index) const
    {
        ...
        return property(curr_prop, schema_);
    }
